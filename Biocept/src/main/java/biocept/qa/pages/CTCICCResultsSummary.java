...
public class CTCICCResultsSummary extends BioceptBase {
	...
	public void ctcResult() {
		try {
			WebElement optionsText = explicitWait.waitUntilElementToBeVisible(ctcResultSection)
					.findElement(By.cssSelector("select[data-bind*='optionsText']"));
	...
	public void ctcTestImage() {
		...
		WebElement ctcImageButton = ctcResultSection
				.findElement(By.cssSelector("button[data-bind*='getImagesFromAzure']"));
	...
	public void iccResult() {
		List<WebElement> optionsText = iccResultSection
				.findElements(By.cssSelector("select[data-bind*='optionsText']"));
	...
	public void iccTestImage() {
		List<WebElement> iccImageButton = iccResultSection
				.findElements(By.cssSelector("button[data-bind*='getImagesFromAzure']"));
		...
		if (iccImageButton.size() > 0) {
			try {
				for (int i = 0; i < iccImageButton.size(); i++) {
					...
					// String CIValue=
					// explicitWait.waitUntilElementToBeClickable(ChooseImagesElement).getText();
					...
					for (int l = i + 1; l < iccClose.size(); l++) {
						...
						// String CValue=
						// explicitWait.waitUntilElementToBeClickable(ICCCloseElement).getText();
						//
						//
						// if(CValue.equalsIgnoreCase("Close"))
						// {
						// ICCCloseElement.click();
						// WebDriverWait wait_fadeIn=new WebDriverWait(driver,
						// 05);
						// wait_fadeIn.until(ExpectedConditions.invisibilityOf(fadeIn));
						//
						// break;
	...
	// public void iccTestImage () {
	// List<WebElement> iccImageButton =
	// iccResultSection.findElements(By.cssSelector("button[data-bind*='getImagesFromAzure']"));
	//
	// if(iccImageButton.size()>0){
	// try {
	// for(int i=0;i<iccImageButton.size();i++){
